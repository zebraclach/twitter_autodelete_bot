import os
import json
import time
import tweepy
import threading
from flask import Flask

# 1. Renderの監視を回避するためのダミーサーバー
app = Flask(__name__)

@app.route('/')
def health_check():
    return "Bot is running", 200

# 2. 削除ロジック（古い順ソート付き）
def start_delete():
    time.sleep(10) # 起動待ち
    
    file_path = 'delete_list.json'
    if not os.path.exists(file_path):
        print("Error: delete_list.json が見つかりません。")
        return

    with open(file_path, 'r') as f:
        target_ids = json.load(f)

    if not target_ids:
        print("削除対象のツイートはありません。")
        return

    # 【重要】IDを数字として小さい順（＝投稿が古い順）に並べ替える
    target_ids.sort(key=int)

    print(f"削除作業を【古い順】に開始します。残り: {len(target_ids)}件")

    count = 0
    remaining_ids = target_ids.copy()

    # X API 認証
    client = tweepy.Client(
        consumer_key=os.environ.get('TWITTER_API_KEY'),
        consumer_secret=os.environ.get('TWITTER_API_SECRET'),
        access_token=os.environ.get('TWITTER_ACCESS_TOKEN'),
        access_token_secret=os.environ.get('TWITTER_ACCESS_TOKEN_SECRET')
    )

    for tweet_id in target_ids:
        if count >= 45: # 1回の上限
            print(f"本日の削除上限(45件)に達しました。")
            break
        try:
            client.delete_tweet(tweet_id)
            print(f"削除成功: {tweet_id}")
            remaining_ids.remove(tweet_id)
            count += 1
            time.sleep(20) # 429エラー対策で20秒待機
        except Exception as e:
            print(f"エラー (ID:{tweet_id}): {e}")
            if "429" in str(e): # 回数制限
                print("X APIの回数制限に達しました。本日は終了します。")
                break
            if "401" in str(e): # 認証エラー
                break

    # 更新されたリストを保存
    with open(file_path, 'w') as f:
        json.dump(remaining_ids, f)
    print(f"今回の作業を終了しました。残り: {len(remaining_ids)}件")

if __name__ == "__main__":
    threading.Thread(target=start_delete, daemon=True).start()
    port = int(os.environ.get("PORT", 10000))
    app.run(host='0.0.0.0', port=port)
